 ...
 * needed to restore one flexible format course.
 */
class restore_format_flexible_plugin extends restore_format_plugin {
    ...
    protected function need_restore_numsections() {
        ...
        /* Post 3.3 may or may not have numsections in the backup depending on the version.
           of Flexible used.  So use the existance of 'numsections' in the course.xml
    ...
    protected function define_course_plugin_structure() {
        ...
        $elename = 'flexible'; // This defines the postfix of 'process_*' below.
        /*
         * This is defines the nested tag within 'plugin_format_flexible_course' to allow '/course/plugin_format_flexible_course' in
    ...
    /**
     * Process the 'plugin_format_flexible_course' element within the 'course' element in the 'course.xml' file in the '/course'
     * folder of the zipped backup 'mbz' file.
     */
    public function process_flexible($data) {
        global $DB;

        $data = (object) $data;

        /* We only process this information if the course we are restoring to
           has 'flexible' format (target format can change depending of restore options). */
    ...
    protected function define_section_plugin_structure() {
        ...
        // Add own format stuff.
        $elename = 'flexiblesection'; // This defines the postfix of 'process_*' below.
        /* This is defines the nested tag within 'plugin_format_flexible_section' to allow '/section/plugin_format_flexible_section' in
         * the path therefore as a path structure representing the levels in section.xml in the backup file.
         */
        $elepath = $this->get_pathfor('/');
        $paths[] = new restore_path_element($elename, $elepath);
    ...
    /**
     * Process the 'plugin_format_flexible_section' element within the 'section' element in the 'section.xml' file in the
     ...
     * Still need courseid in the 'format_flexible_icon' table as it is used in discovering what records to remove when deleting a
     * course.
     */
    public function process_flexiblesection($data) {
        global $DB;

        $data = (object) $data;

        /* Allow this to process even if not in the flexible format so that our event observer on 'course_restored'
           can perform a clean up of restored flexible image files after all the data is in place in the database
        ...
        if (!$DB->record_exists('format_flexible_icon', array('courseid' => $data->courseid, 'sectionid' => $data->sectionid))) {
            if (!$DB->insert_record('format_flexible_icon', $data, true)) {
                throw new moodle_exception('invalidrecordid', 'format_flexible', '',
                'Could not insert icon.  Flexible format table format_flexible_icon is not ready.'.
                '  An administrator must visit the notifications section.');
            }
        } else {
            $old = $DB->get_record('format_flexible_icon', array('courseid' => $data->courseid, 'sectionid' => $data->sectionid));
            ...
            if (is_null($old->image)) {
                ...
                $data->id = $old->id;
                if (!$DB->update_record('format_flexible_icon', $data)) {
                    throw new moodle_exception('invalidrecordid', 'format_flexible', '',
                    'Could not update icon.  Flexible format table format_flexible_icon is not ready.'.
                    '  An administrator must visit the notifications section.');
                }
            }
        }
    ...
    public function after_restore_course() {
        ...
        $data = $this->connectionpoint->get_data();
        $backupinfo = $this->step->get_task()->get_info();
        if ($backupinfo->original_course_format !== 'flexible') {
