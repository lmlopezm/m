...
Free Software
=============
The Flexible format is 'free' software under the terms of the GNU GPLv3 License, please see 'COPYING.txt'.

The primary source for downloading this branch of the format is https://moodle.org/plugins/view.php?plugin=format_flexible
with 'Select Moodle version:' set at 'Moodle 3.6'.

The secondary source is a tagged version with the v3.6 prefix on https://github.com/gjb2048/moodle-courseformat_flexible/tags

If you download from the development area - https://bitbucket.org/covuni/moodle-format_flexible - consider that
...
Support
=======
The Flexible format comes with NO support.  If you would like support from me (Gareth) then I'm happy to provide it
...
3. Copy 'flexible' to '/course/format/' if you have not already done so.
4. Go back in as an administrator and follow standard the 'plugin' update notification.  If needed, go to
   'Site administration' -> 'Notifications' if this does not happen.
5. Put Moodle out of Maintenance Mode.
6. You may need to check that the permissions within the 'flexible' folder are 755 for folders and 644 for files.
...
3. In '/course/format/' remove the folder 'flexible'.
4. In the database, remove the row with the 'plugin' of 'format_flexible' and 'name' of 'version' in the 'config_plugins' table
   and drop the 'format_flexible_icon' and 'format_flexible_summary' tables.
5. Put Moodle out of Maintenance Mode.
...
3. In '/course/format/' move old 'flexible' directory to a backup folder outside of Moodle.
4. Copy new 'flexible' to '/course/format/'.
5. Go back in as an administrator and follow standard the 'plugin' update notification.  If needed, go to
   'Site administration' -> 'Notifications' if this does not happen.
6. If you have upgraded from Moodle 1.9 and were using the Flexible format there, please follow 'Upgrading from M1.9' below
...
1.  Put Moodle in 'Maintenance Mode' so that there are no users using it bar you as the administrator.
2.  In '/course/format/' remove the folder 'flexible' i.e. ALL it's contents - this is VITAL.
3.  Put in the replacement 'flexible' folder into '/course/format/'.
...
4.1 In the database, remove the row with the 'plugin' of 'format_flexible' and 'name' of 'version' in the 'config_plugins' table
    and drop the 'format_flexible_icon' and 'format_flexible_summary' tables.  If automatic 'Purge all caches' appears not to work by
    lack of display etc. then perform a manual 'Purge all caches' under 'Home -> Site administration -> Development ->
    Purge all caches'.
4.2 In the database, change the row with the 'plugin' of 'format_flexible' and 'name' of 'version' in the 'config_plugins' table
    to have the same 'value' as '$plugin->version' in the 'flexible/version.php' file i.e. like '2013083000'.  Then perform a manual
...
are always the same, so for Moodle 3.6.x there will be a Flexible format 3.6.x.  The primary release area is located on
https://moodle.org/plugins/view.php?plugin=format_flexible.  It is also essential that you are operating the required version of Moodle
as stated at the top - this is because the format relies on core functionality that is out of its control.

All 'Flexible format' does is integrate with the course page and control it's layout, therefore what may appear to be an issue
with the format is in fact to do with a theme or core component.  Please be confident that it is an issue with 'Flexible format'
but if in doubt, ask.
...
on https://bitbucket.org/covuni/moodle-format_flexible/issues or contact me direct (details at the bottom).
...
Viewing
-------
Click on a flexible icon or use 'esc' to open the current selected icon which will then display the shade box containing the section
...
Accessibility
-------------
If you wish for a user not to see the flexible and shadebox and have the course presented like the Topics format, then add a custom
...
https://github.com/gjb2048/moodle-format_flexible/issues/23
...
----------------------------------
To change the 'flexiblekeys.js' code then you'll firstly need to read: http://docs.moodle.org/dev/YUI/Shifter
it is used to build the source in '/yui/src/flexiblekeys/js/flexiblekeys.js' and bring in the 'gallery-event-nav-keys' to build
the YUI module into 'yui/build/moodle-format_flexible-gridkeys' and place a back port minified version in '/yui/gridkeys' for
...
$PAGE->requires->yui_module('moodle-format_flexible-gridkeys', 'M.format_flexible.gridkeys.init', null, null, true);
...
-----------------------
Edit 'styles.css', change the value in the '.course-content ul.flexibleicons li.currentselected' selector and perform a 'Purge all caches'
or override in your theme.

Current section
---------------
Edit 'styles.css', change the value in the '.course-content ul.flexibleicons li.current' selector and perform a 'Purge all caches' or
...
Languages
---------
The flexible/lang folder contains the language files for the format.
...
Styles
------
The file flexible/styles.css contains the CSS styles for the format which can
...
flexible/backup/moodle2/backup_format_flexible_plugin.class.php
flexible/backup/moodle2/restore_format_flexible_plugin.class.php
